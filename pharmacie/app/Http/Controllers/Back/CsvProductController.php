...
class CsvProductController extends Controller
{
    ...
    public function orderExport()
    {
        ...
        $lists =  Order::join('users', 'orders.user_id', '=', 'users.id')
        ->select('orders.id as id_order', 'users.last_name as name', 'users.first_name as prenom', 'users.email as email', 'users.phone as user_phone', 'users.ship_address1 as user_adresse', 'orders.cart as cart', 'orders.total_amount as total_amount')
        ->get()
        ->each(function ($order) {
            // Décodez la chaîne JSON dans un tableau associatif
            $cartData = json_decode($order->cart, true);

            // Extrait uniquement les produits du panier
            $products = collect($cartData)->map(function ($item, $key) {
                return $item['name']; // Ou toute autre information que vous voulez extraire
            });

            // Mettez à jour la colonne 'cart' avec les nouveaux produits
            $order->update(['cart' => json_encode($products)]);
        })->toArray();
